...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponseDTO<Map<String, String>>> handleMultipartException(
        ...
        ApiResponseDTO<Map<String, String>> response = new ApiResponseDTO<>(false, message, details, getCurrentTimestamp());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<ApiResponseDTO<Map<String, String>>> handleNoSuchAlgorithm(
        ...
        log.error("Cryptographic algorithm not available on request: {} {}", request.getMethod(), request.getRequestURI(), ex);
    ...
    public ResponseEntity<ApiResponseDTO<Map<String, String>>> handleRuntime(
        ...
        String message = Optional.ofNullable(ex.getMessage()).filter(msg -> !msg.isBlank())
                .orElse("A runtime error occurred while processing your request");
    ...
    public ResponseEntity<ApiResponseDTO<Map<String, String>>> handleGeneric(
        ...
        log.error("Unexpected error [{}] on request: {} {}", errorId, request.getMethod(), request.getRequestURI(), ex);
    ...
    private HttpStatus determineStatusFromMessage(String message) {
        ...
        if (lower.contains("not found") || lower.contains("does not exist")) return HttpStatus.NOT_FOUND;
        if (lower.contains("duplicate") || lower.contains("already exists")) return HttpStatus.CONFLICT;
        if (lower.contains("unauthorized") || lower.contains("access denied")) return HttpStatus.FORBIDDEN;

        return HttpStatus.BAD_REQUEST;
    }

    // =========================================================================
    // CUSTOM DOMAIN EXCEPTIONS (optional)
    // =========================================================================
    ...
    public static class TextEntryNotFoundException extends RuntimeException {
        public TextEntryNotFoundException(String message) { super(message); }
    }

    @ExceptionHandler(TextEntryNotFoundException.class)
    public ResponseEntity<ApiResponseDTO<Object>> handleTextEntryNotFound(TextEntryNotFoundException ex) {
...
//
//import com.app.backend.dto.ApiResponseDTO;
//import com.app.backend.dto.ErrorResponseDTO;
//import jakarta.servlet.http.HttpServletRequest;
//import jakarta.validation.ConstraintViolation;
//import jakarta.validation.ConstraintViolationException;
//import lombok.extern.slf4j.Slf4j;
